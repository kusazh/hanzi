# Hanzi Suika Game 合成大漢字

https://github.com/user-attachments/assets/044e8733-4973-4495-b7cd-1932fd0d3214

## Changelog

### 0.3
- Open source dictionary-generating tools.
- Revise parts (unfinished).

### 0.2
- Use font subset to reduce size.
- Add theme color for PWA.
- New level-up and scoring rule.

### 0.1
- First release.

## Acknowledgements

- [CHISE](https://www.chise.org)
- [Mandarin Chinese Mini Dictionary](https://dict.mini.moe.edu.tw)
- [Matter.js](https://github.com/liabru/matter-js/blob/master/LICENSE)
- [Opentype.js](https://github.com/opentypejs/opentype.js/blob/master/LICENSE)
- [Source Han Serif](https://github.com/adobe-fonts/source-han-serif/blob/release/LICENSE.txt)
